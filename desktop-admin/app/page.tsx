...
export default function Operations(){
 ...
 return <>{user&&<HomeEditor online userName={user.name} onLogout={()=>void logout()}/>}{(!user||expired)&&<div className={'login-page '+(user?'session-overlay':'')}><aside className="login-story"><div className="login-brand">TOPUYI<span>商家运营工作台</span></div><div><span className="login-eyebrow">让品牌首页，常看常新</span><h1>好内容，<br/>从这里上场。</h1><p>一处管理页面、图片与品牌素材。<br/>把更多时间留给创意和运营。</p><div className="login-features"><span><PanelsTopLeft/>自由组合页面</span><span><ImagePlus/>上传品牌素材</span><span><Users/>团队共享草稿</span></div></div><small>TOPUYI · 让热爱发光</small></aside><main className="login-side"><form className="login-card" onSubmit={login}><span className="login-icon"><LockKeyhole/></span><h2>{user?'重新登录':'登录运营后台'}</h2><p>使用管理员为你开通的运营账号</p><label htmlFor="ops-user">运营账号<Input id="ops-user" autoComplete="username" required value={username} onChange={e=>setUsername(e.target.value)} placeholder="请输入账号" maxLength={64}/></label><label htmlFor="ops-password">登录密码<Input id="ops-password" type="password" autoComplete="current-password" required value={password} onChange={e=>setPassword(e.target.value)} placeholder="请输入密码" maxLength={64}/></label>{error&&<p className="login-error" role="alert">{error}</p>}<Button type="submit" disabled={busy}>{busy?'正在登录…':'进入工作台'}<ArrowRight/></Button><div className="login-help">账号开通或密码重置，请联系后台管理员。</div><div className="login-scope"><ShieldCheck size={18}/><span>当前仅连接开发版首页<br/>保存与同步不会发布微信新版小程序</span></div></form></main></div>}</>;
}
