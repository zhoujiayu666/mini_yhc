...
const rhythmBehaviors = {
  ...
  onConnectionLost() {
    if (this.data.isListening || this.data.recordingPending) {
      this.stopAudioListener();
    }
  }
};

const RHYTHM_SKIP_KEYS = new Set(['onLoad', 'onShow', 'onHide', 'onUnload']);

function attachRhythm(page) {
  Object.keys(rhythmBehaviors).forEach((key) => {
    if (RHYTHM_SKIP_KEYS.has(key) || typeof rhythmBehaviors[key] !== 'function') {
      return;
    }
    if (key === 'showHelp') {
      page.showRhythmHelp = rhythmBehaviors.showHelp.bind(page);
      page.closeRhythmHelp = rhythmBehaviors.closeHelp.bind(page);
      page.stopRhythmPropagation = rhythmBehaviors.stopPropagation.bind(page);
      return;
    }
    page[key] = rhythmBehaviors[key].bind(page);
  });
  rhythmBehaviors.onLoad.call(page);
}

module.exports = {
  RHYTHM_INITIAL_DATA,
  attachRhythm,
  rhythmOnHide: rhythmBehaviors.onHide,
  rhythmOnUnload: rhythmBehaviors.onUnload,
  rhythmOnConnectionLost: rhythmBehaviors.onConnectionLost
};
