...
function context(){
  ...
  return {accessCode,preview,key:preview?PREVIEW_CACHE:LIVE_CACHE,catalogSource:preview?'home_dev':'home_live'};
}
function peek(){
  const {accessCode,key,catalogSource}=context();
  try{
    const cached=wx.getStorageSync(key);
    if(cached&&cached.code===accessCode&&cached.config?.version===1&&Array.isArray(cached.config.blocks)&&Array.isArray(cached.products)){
      return {config:cached.config,products:cached.products,catalogSource,status:'已读取本地内容',fromCache:true};
    }
  }catch(_){}
  return null;
}
async function load(){
  const {accessCode,preview,key,catalogSource}=context();
  const token=key+':'+accessCode;
  ...
  const promise=(async()=>{
    try{
      ...
      const cached=peek();
      return {config:cached?cached.config:empty(),products:cached?cached.products||[]:[],catalogSource,status:'加载失败，请下拉刷新',error:error.message||'请检查网络后重试'};
    }
  })();
  inflight={token,promise};
  try{return await promise;}finally{if(inflight&&inflight.promise===promise)inflight=null;}
}
module.exports={getCode,setCode,clearCode,load,peek};
