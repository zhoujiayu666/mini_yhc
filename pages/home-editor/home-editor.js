...
Page({
  ...
  field(e) {
    ...
    if (field === 'height') value = Math.max(160, Math.min(900, Number(value) || 340));
  ...
  addItem() { const blocks = t.clone(this.data.config.blocks); if (blocks[this.data.selected].items.length >= 20) return; blocks[this.data.selected].items.push(t.item('新内容')); this.update(blocks); },
