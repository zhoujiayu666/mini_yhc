...
function loadOptionalKey(modulePath, field) {
  try {
    const mod = require(modulePath);
    return String((mod && mod[field]) || '').trim();
  } catch (e) {
    return '';
  }
}

/** 优先级：云函数环境变量 > app-config > 本地 wecom-local.js（勿提交仓库） */
function getWebhookKey() {
  return (
    process.env.WECOM_ORDER_WEBHOOK_KEY ||
    process.env.WECOM_WEBHOOK_KEY ||
    loadOptionalKey('./app-config', 'WECOM_ORDER_WEBHOOK_KEY') ||
    loadOptionalKey('./wecom-local', 'WECOM_ORDER_WEBHOOK_KEY') ||
    ''
  ).trim();
}
